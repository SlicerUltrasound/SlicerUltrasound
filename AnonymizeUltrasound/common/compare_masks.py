...
def calculate_segmentation_metrics(
    ...
    :param include_hausdorff: whether to include Hausdorff distance calculation
    :return: dictionary with segmentation metrics
    """
    ...
    # Hausdorff distance (if both masks have any positive pixels)
    if np.any(gt_bin) and np.any(pred_bin) and include_hausdorff:
        gt_pts = np.argwhere(gt_bin)
        pred_pts = np.argwhere(pred_bin)
        hd = max(directed_hausdorff(gt_pts, pred_pts)[0], directed_hausdorff(pred_pts, gt_pts)[0])
    else:
        hd = np.nan
